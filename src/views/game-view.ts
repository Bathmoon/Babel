...
export class GameView extends BaseView {
  ...
  handleEnemyTurns() {
    this.gameMap.actors.forEach((entity) => {
      if (entity.canAct) {
        try {
          entity.ai?.perform(entity, this.gameMap);
        } catch {}
      }
    });
  }
  ...
  private static loadGame(
    ...
    for (let entity of parsedMap.entities) {
      if (entity.name === "Orc") {
        const orc = spawnOrc(map, entity.x, entity.y);

        orc.fighter.hp = entity.fighter?.hp || orc.fighter.hp;

        if (entity.aiType === "confused") {
          orc.ai = new ConfusedEnemy(orc.ai, entity.confusedTurnsRemaining);
        }
      } else if (entity.name === "Troll") {
        const troll = spawnTroll(map, entity.x, entity.y);

        troll.fighter.hp = entity.fighter?.hp || troll.fighter.hp;

        if (entity.aiType === "confused") {
          troll.ai = new ConfusedEnemy(troll.ai, entity.confusedTurnsRemaining);
        }
      } else if (entity.name === "Health Potion") {
        spawnHealthPotion(map, entity.x, entity.y);
      } else if (entity.name === "Lightning Scroll") {
        spawnLightningScroll(map, entity.x, entity.y);
      } else if (entity.name === "Confusion Scroll") {
